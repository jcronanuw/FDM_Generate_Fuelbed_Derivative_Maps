...
for(aa in 1:length(run_in))
  ...
  for(z in 1:length(fccsVar_col))
  {
    ...
    for(i in 1:length(intervals2))
    {
      ...
      {
        maps_out[[i]][maps_in2[[i]] == zeros[b]] <- assignments[b]
      }
    }
    ...
    test_results <- data.frame(map = intervals2, 
                               multiples = rep(0,length(intervals2)), 
                               fuelbeds_no_fccsVar = rep(0,length(intervals2)), 
                               fuelbeds_na__fccsVar = rep(0,length(intervals2)),
                               na_fuelbeds = rep(0,length(intervals2)))
    ...
    for(c in 1:length(intervals2))
    {
      ...
      test_results[c,2] <- ifelse(any(unique(crosswalk_values) %in% c(0,1)),"OK", "ERROR")
      
      #Results should be zero
      test_results[c,3] <- ifelse(length(test_base[test_base %in% new_fb[crosswalk_values == 0]]) == 0, 
                                  "OK", "ERROR")
      
      test_results[c,4] <- ifelse(length(test_var[is.na(test_var) == T]) == length(test_base[test_base == -9999]), 
                                  "OK", "ERROR")
      
      na_fuelbeds_possible_errors_if_this_contains_values[[c]] <- sort(unique(test_base[is.na(test_var) == T]))
      if(length(na_fuelbeds_possible_errors_if_this_contains_values[[c]]) > 1)
      { } else
      {
        test_results[[c,5]] <- na_fuelbeds_possible_errors_if_this_contains_values[[c]]
        na_fuelbeds_possible_errors_if_this_contains_values[[c]] <- 0
      }
    }
    ...
    {
      maps_out_change[[i]][maps_in2[[1]] %in% zeros] <- -8888
      maps_out_change[[i]][maps_in2[[1]] == -9999] <- -9999
    }
  ...
  for(y in 1:length(fuelbedVar_name))
  {
    ...
    for(i in 1:length(intervals2))
    {
      filenames_out_fuelbed[i] <- paste(fuelbedVar_out[y], rx_fire, "_", run_out, "_", intervals2[i], ".asc", sep = "")
    }
